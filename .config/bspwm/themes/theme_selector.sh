#!/bin/bash
THEME="Default"
readonly wall_dir="$HOME/.config/bspwm/themes/previews/"
...
rofi_command="rofi -dmenu -theme ~/.config/bspwm/themes/ThemeSelector.rasi -theme-str $rofi_override"
...
wall_selection=$(find "${wall_dir}" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.webp" \) -print0 |
    xargs -0 basename -a | tr '-' ' ' | awk '{ print $1 }' | 
    LC_ALL=C sort |
    while IFS= read -r A; do
        printf '%s\000icon\037%s/%s-bspwm.jpg\n' "$A" "$cacheDir" "$A" # mocha-bspwm.jpgx
    done | $rofi_command)
...
if [[ ! -z "${wall_selection}" ]]; then 
  $HOME/.config/bspwm/themes/theme.sh "${wall_selection}"
fi
